...
async def spit_log(msg, title=f"New Log - {datetime.now().astimezone().strftime('%d/%m/%Y %H:%M:%S %Z')}", status=Status.unknown):
    log_channel = bot.get_channel(LOG_CHANNEL_ID)
    # spits a line of log in the logging channel
    if msg:
        embedded_message = discord.Embed(
            title=title, description=msg, color=status.value)
...
async def init_onboard(member):
    ...
    await spit_log(f'{member.mention} pending role assigned. Executing onboarding flow.',
                   title=f"⏬ Starting to onboard {member.name}... Step 1 complete!", status=Status.success)
    ...
    channel_config_overwrites = {
        ...
        pending_role: discord.PermissionOverwrite(view_channel=False),
        onboard_role: discord.PermissionOverwrite(
            view_channel=True, read_messages=True, send_messages=True)
    }
    verification_category = discord.utils.get(
        guild.categories, id=VERIFICATION_CATEGORY_ID)
    onboard_channel = await guild.create_text_channel(name=code, category=verification_category, overwrites=channel_config_overwrites)
    await spit_log(f'Role {code} created and assigned for {member.mention}.',
                   title=f"⏬ Onboarding {member.name}... Step 2 complete!", status=Status.success)

    # 3. send welcome messages
    welcome_message = await onboard_channel.send(f'Welcome to **Build the Future Community**, {member.mention}! ' +
    ...
    welcome_pending_log = await spit_log(f'{member.mention} ready to start onboarding. Waiting for reaction...',
                                         title=f"⏸ Onboarding {member.name}... Step 3 pending...", status=Status.warning)
    ...
    await spit_log(f'{member.mention} onboarding started!',
                   title=f"⏬ Onboarding {member.name}... Step 3 complete!", status=Status.success)

    await pause(onboard_channel, 2)
    await onboard_channel.send('In order to allow you to enter our Discord community, we need to complete a few quick onboarding steps...')
    await pause(onboard_channel, 1)
    await onboard_channel.send('First, please read our **Code of Conduct**. Breaking any rules in the Code of Conduct will result in your prompt removal from the BTF Community.')
    await pause(onboard_channel, 1)
    await onboard_channel.send('https://docs.google.com/document/d/1H9ammGSeypqZufuMT3Qn8Lcb-gBPxxoZ3PjDrDcUZis/edit')
    await pause(onboard_channel, 2)
    await onboard_channel.send('**Please respond with `agree` to acknowledge the Code of Conduct.**')

    # 4. wait for acknowledgement on CoC
    def check_coc(m):
        return "agree" in m.content.lower() and m.channel == onboard_channel

    coc_pending_log = await spit_log(f'Conduct agreement sent to the welcome channel for {member.mention}. Awaiting acknowledgement...',
                                     title=f"⏸ Onboarding {member.name}... Step 4 pending...", status=Status.warning)

    coc_pending = await bot.wait_for('message', check=check_coc)
    await onboard_channel.send(f"Thank you for your agreement, {member.name}!".format(coc_pending))
    await coc_pending_log.delete()

    await spit_log(f'Conduct acknowledged by {member.mention}!',
                   title=f"⏬ Onboarding {member.name}... Step 4 complete!", status=Status.success)

    # 5. wait for email address
    await pause(onboard_channel, 2)
    await onboard_channel.send('Now, we need to verify your email address.')
    email_pending_log = await spit_log(f'Email prompt sent to the welcome channel for {member.mention}. Awaiting email...',
                                       title=f"⏸ Onboarding {member.name}... Step 5 pending...", status=Status.warning)
    ...
    async def email_check_bump():
        ...
        """
        await pause(onboard_channel, 1)
        await onboard_channel.send('**Please reply here with your @berkeley.edu email address.**')

        email_pending = await bot.wait_for('message', check=lambda m: m.channel == onboard_channel)
    ...
    while not (valid_email(email) and valid_domain(email)):
        ...
        if not valid_email(email):
            await onboard_channel.send(f'**Invalid response** - are you sure this is an email?')
        elif not valid_domain(email):
            await onboard_channel.send(f'**Invalid domain.** Your email must end with {VALID_DOMAIN}.')

        email = await email_check_bump()

    await onboard_channel.send(f'Success! We will send an email to {email}. (WIP)')
    await email_pending_log.delete()

    await spit_log(f'Email collected from {member.mention}: {email}',
                   title=f"⏬ Onboarding {member.name}... Step 5 complete!", status=Status.success)
...
async def on_reaction_add(reaction, user):
    ...
    if message in WATCH_SOS and reaction.emoji == "🆘" and user != bot.user:
        ...
        support_role = discord.utils.get(
            user.guild.roles, id=SUPPORT_ROLE_ID)
        await message.channel.send(f"Help requested! A staff member will be here to help you soon. {support_role.mention}")
        await spit_log(f'{user.name} just requested help in {message.channel.mention}!', title=f"🆘 Help requested by {user.name}!", status=Status.error)
...
async def reset_server(ctx):
    ...
    for role in ctx.guild.roles:
        if role.name.startswith("o-"):
            try:
                ...
                await spit_log(f'The role {role.name} cannot be removed!', status=Status.error)

    if len(removed_roles) > 0:
        await spit_log(f'Roles {", ".join(removed_roles)} have been swept up!', status=Status.success)
    else:
    ...
    removed_channels = []
    verification_category = discord.utils.get(
        ctx.guild.categories, id=VERIFICATION_CATEGORY_ID)
    for channel in verification_category.channels:
        await channel.delete()
        removed_channels.append(channel.name)

    if len(removed_channels) > 0:
        await spit_log(f'Channels {", ".join(removed_channels)} have been swept up!', status=Status.success)
    else:
